...
module.exports = function(socket, connections) {
  ...
  socket.on('login', function(data){
    User.findOne({username: data.username, is_persona: false}, function(err, user) {
      if(user){
  ...
  socket.on('login_persona', function(data) {
    var post_data = {assertion: data.assertion, audience: 'http://localhost:3000'}
    $.post('https://verifier.login.persona.org/verify', post_data, function(data) {
      User.findOne({username: data.email, is_persona: true}, function(err, user) {
        if (user) {
          var exists;
          current_user = user;
          if (connections[user.username] !== undefined) {
